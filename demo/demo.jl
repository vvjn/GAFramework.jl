...
m = EuclideanModel(x -> abs(x[1] + 2x[2] + 3x[3] + 4x[4] + 5x[5] - 42),
...
# Minimizes exp(|x|)
m = EuclideanModel(x -> exp(dot(x,x)),
...
m = EuclideanModel(x -> abs(tanh(x[1]+1000)), -2000., 2000.)
...
# It's possible to also change the other fields but it doesn't really make too much sense to
